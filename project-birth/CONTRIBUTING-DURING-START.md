...
As for how to successfully Decaffeinate the core of the editor (or any other repos/packages in use), a great template for successfully doing so can be seen on [`pulsar-edit/pulsar` PR #13](https://github.com/pulsar-edit/pulsar/pull/13). But a huge thing to keep in mind is that when Atom was originally created they had used tooling for CoffeeScript 1.x whereas much of the tooling thats available now is CoffeeScript 2.x, what this means is that in order to decaffeinate without breaking changes tooling for version 1.x **MUST** be used. Otherwise there can be unknown ramifications. The best tool, that we have found, is to use [`decaffeinate/decaffeinate` Version 4.0.0](https://github.com/decaffeinate/decaffeinate/releases/tag/v4.0.0). This is the stable release of their decaffeination tooling, that uses CoffeeScript 1.x. Anything beyond that could cause breaking changes. But as long as you are able to ensure that the tooling you use, focuses on CoffeeScript 1.x then no worries. 

Otherwise after using this tooling, while the JavaScript generated won't be the prettiest, it will be functional. And from that functional JavaScript you can change any declarations of `var` to `let` or `const` as needed. And this can always be taken further if needed, but this alone should be enough to successfully decaffeinate. 
